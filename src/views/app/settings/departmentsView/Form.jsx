 ...
 */
const DepartmentForm = ({ container = "drawer", refreshSelect, openDialog, setOpenDialog }) => {
   const { setIsLoading } = useGlobalContext();
   const { singularName, department, formTitle, setFormTitle, textBtnSubmit, setTextBtnSubmit, formikRef, isEdit, setIsEdit, createOrUpdateDepartment } =
   ...
   const formData = [
      ...
      },
      {
         ...
         input: (
            <Input
               key={`key-input-letters`}
               col={12}
               idName="letters"
               label="Letras del departamento"
               placeholder="Escriba las letras que identifiquen al departamento"
               type="text"
               textStyleCase={true}
               helperText="Estas letras se pondran en el folio para identificar más rápido al departamento que corresponde"
               required
            />
         ),
         value: "",
         validations: Yup.string().trim().required("Letras del departamento requerido"),
         validationPage: [],
         dividerBefore: { show: false, title: "", orientation: "horizontal", sx: {} }
      },
      {
         ...
         value: "",
         validations: Yup.string().trim().required("Nombre del departamento requerido"),
         validationPage: [],
         dividerBefore: { show: false, title: "", orientation: "horizontal", sx: {} }
      },
      {
         ...
         input: (
            <Textarea
               key={`key-input-department_description`}
               col={12}
               idName="department_description"
               label="Descripción"
   ...
   const onSubmit = async (values, { setSubmitting, resetForm }) => {
      ...
      setIsLoading(true);
      const res = await createOrUpdateDepartment(values);
