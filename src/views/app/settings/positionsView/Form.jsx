 ...
 */
const PositionForm = ({ container = "drawer", refreshSelect, openDialog, setOpenDialog }) => {
   const { auth } = useAuthContext();
   const { setIsLoading } = useGlobalContext();
   const { singularName, position, formTitle, setFormTitle, textBtnSubmit, setTextBtnSubmit, formikRef, isEdit, setIsEdit, createOrUpdatePosition } =
      usePositionContext();
   const { setDepartmentsSelect, getSelectIndexDepartments } = useDepartmentContext();

   const [checkAdd, setCheckAdd] = useState(checkAddInitialState);
   // const [departmentFormDialog, setDepartmentFormDialog] = useState(false);

   // const { refetch: refreshDepartments } = useFetch(getSelectIndexDepartments, setDepartmentsSelect);
   ...
   const formData = [
      ...
      },
      {
         ...
         input: (
            <Input
               key={`key-input-position`}
               col={12}
               idName="position"
               label="Nombre del puesto de trabajo"
               placeholder="Escriba el nombre del puesto de trabajo"
               type="text"
               textStyleCase={null}
               helperText=""
               required
      ...
      },
      {
         ...
         input: (
            <Textarea
               ...
               idName={"description"}
               label={"Describir el puesto"}
               placeholder={"Describa brevemente que hace el puesto"}
               helperText={""}
               textStyleCase={null}
               styleInput={"classic"}
               size={"md"}
   ...
   const onSubmit = async (values, { setSubmitting, resetForm }) => {
      ...
      setIsLoading(true);
      const res = await createOrUpdatePosition(values);
