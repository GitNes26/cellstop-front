...
const SellerDT = () => {
   ...
   const { singularName, allSellers, setFormTitle, setTextBtnSubmit, formikRef, setIsEdit, deleteSeller, disEnableSeller, getAllSellers, getSeller } =
      useSellerContext();
   const mySwal = withReactContent(Swal);

   //#region COLUMNAS
   const fontSizeTable = { text: "sm", subtext: "xs" };
   const globalFilterFields = ["letters", "seller", "seller_description", "active", "created_at"];
   ...
   const handleClickEdit = async (id) => {
      try {
         ...
         if (res.result.seller_description) res.result.seller_description == null && (res.result.seller_description = "");
   ...
   const handleClickDisEnable = async (id, name, active) => {
      try {
         setTimeout(async () => {
            setIsLoading(true);
            const res = await disEnableSeller(id, !active);
            // console.log('🚀 ~ handleClickLogout ~ res:', res);
            if (!res) return setIsLoading(false);
            if (res.errors) {
               setIsLoading(false);
   ...
   const formatData = async () => {
      try {
         // console.log("cargar listado", allSellers);
         await allSellers.map((obj, index) => {
            // console.log(obj);
            let register = obj;
            register.key = index + 1;
            // register.actions = <ButtonsAction id={obj.id} name={obj.seller} active={obj.active} />;
            register.actions = [
               { label: "Editar", iconName: "Edit", tooltip: "", handleOnClick: () => handleClickEdit(obj.id), color: "blue" },
               { label: "Eliminar", iconName: "Delete", tooltip: "", handleOnClick: () => handleClickDelete(obj.id, obj.seller), color: "red" }
            ];
            data.push(register);
         });
      } catch (error) {
