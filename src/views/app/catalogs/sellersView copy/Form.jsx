 ...
 */
const SellerForm = ({ container = "drawer", refreshSelect, openDialog, setOpenDialog }) => {
   const { setIsLoading } = useGlobalContext();
   const { singularName, seller, formTitle, setFormTitle, textBtnSubmit, setTextBtnSubmit, formikRef, isEdit, setIsEdit, createOrUpdateSeller } =
      useSellerContext();
      ...
      },
      {
         ...
         input: (
            <Input
               ...
               col={12}
               idName="letters"
               label="Letras del vendedor"
               placeholder="Escriba las letras que identifiquen al vendedor"
               type="text"
               textStyleCase={true}
               helperText="Estas letras se pondran en el folio para identificar más rápido al vendedor que corresponde"
               required
            />
         ),
         value: "",
         validations: Yup.string().trim().required("Letras del vendedor requerido"),
         validationPage: [],
         dividerBefore: { show: false, title: "", orientation: "horizontal", sx: {} }
      },
      {
         ...
         value: "",
         validations: Yup.string().trim().required("Nombre del vendedor requerido"),
         validationPage: [],
         dividerBefore: { show: false, title: "", orientation: "horizontal", sx: {} }
      },
      {
         ...
         input: (
            <Textarea
               key={`key-input-seller_description`}
               col={12}
               idName="seller_description"
               label="Descripción"
   ...
   const onSubmit = async (values, { setSubmitting, resetForm }) => {
      ...
      setIsLoading(true);
      const res = await createOrUpdateSeller(values);
