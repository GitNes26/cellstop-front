...
import { Box, Card, CardContent, Typography, Button, Alert, useTheme } from "@mui/material";
import {
   Lock,
   Login,
   HowToReg,
...
export default function LoginForms() {
   ...
   const features = [
      { icon: <Dashboard />, text: "Dashboard intuitivo", delay: 0 },
      { icon: <Analytics />, text: "Analíticas en tiempo real", delay: 0.2 },
      { icon: <SupervisedUserCircle />, text: "Gestión de equipos", delay: 0.4 },
      { icon: <Security />, text: "Seguridad enterprise", delay: 0.6 }
   ];
   const [activeTab, setActiveTab] = useState("login");

   const tabs = [
      { id: "login", label: "Iniciar Sesión", icon: <Login /> },
      { id: "register", label: "Registrarse", icon: <HowToReg /> },
      { id: "recover", label: "Recuperar", icon: <Send /> }
   ];
         ...
         <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full max-w-2xl">
            ...
            >
               <div className="flex flex-col lg:flex-row">
                  {/* Tabs laterales para desktop */}
                  <div className="lg:w-1/3 bg-gradient-to-b from-blue-600 to-purple-700 lg:rounded-l-4 rounded-t-4 lg:rounded-tr-none p-8">
                  ...
                  <div className="lg:w-2/3 p-8">
                     <AnimatePresence mode="wait">
                        <motion.div key={activeTab} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                           {/* Contenido del formulario según activeTab */}
                        </motion.div>
                     </AnimatePresence>
                  </div>
               </div>
